...
class Genome:
    ...
    def mutate(
        ...
        """
        Applies weight mutation, then structural mutation (add_node or
        add_connection).

        Weight mutation: standard NEAT practice (Stanley and Miikkulainen,
        2002), each connection independently has a weight_mutate_rate chance
        of being touched, and if touched, a weight_reset_rate chance of
        being reset to a fresh random value instead of perturbed by a small
        Gaussian delta. Defaults (0.8 mutate, 0.1 of that a reset) match the
        paper's typical configuration.

        max_nodes, if given, stops structural growth once the genome already
        has that many nodes (weight mutation still applies). Bounds
        per-individual decode cost, since nothing else in this genome caps
        network size.
        """

        for conn in self.connections.values():
            if random.random() < weight_mutate_rate:
                if random.random() < weight_reset_rate:
                    conn.weight = self._get_random_weight()
                else:
                    conn.weight += random.gauss(0.0, weight_perturb_std)
