...
# TPA fires check_consistency warnings on noisy stochastic simulators — harmless.
# See wiki/CMA-ES_Mirrored_Sampling.md for details.
...
parser.add_argument('--budget', type=int, default=200, help='Number of generations for learning')
...
parser.add_argument('--workers', type=int, default=max(1, os.cpu_count() or 1), help='Number of worker processes for parallel candidate evaluation')
...
def run_vision_simulation(model,
    ...
    trajectory = []
    # Integer step counter avoids float-division drift that caused np.ceil to
    # skip or double-fire a control step after thousands of accumulations.
    step = 0
...
def _build_simulation_context() -> dict[str, Any]:
    """Build one isolated simulation context for a worker process."""
...
def _get_process_context() -> dict[str, Any]:
    """Lazily initialize one Mujoco context per worker process."""
...
def evolve(world, model, data) -> tuple[np.ndarray, int]:
    """Evolve the robot's movement using multiprocessing candidate evaluation."""
    del world, data
    ...
    # Below this threshold covariance adaptation degrades severely.
    min_lambda = 4 + int(3 * np.log(max(num_params, 2)))
    pop_size = max(POP_SIZE, min_lambda)
    # TPA (activated when num_workers > 1) requires paired candidates; odd λ
    # breaks mirrored pairs and triggers check_consistency warnings.
    if pop_size % 2 != 0:
        pop_size += 1

    initial_guess = np.random.uniform(low=-0.5, high=0.5, size=num_params)
    param = ng.p.Array(init=initial_guess)
    # σ⁰ = 0.3·(b−a) = 0.3·1.0 = 0.3 for weights initialised in [−0.5, 0.5].
    # The previous value of 0.075 was 4× too small, limiting reachable solutions
    # to ±0.225 from the initial mean (see wiki/CMA-ES_Parameters.md).
    ...
    console.log(f"Population size: {pop_size} (requested {POP_SIZE}) | Workers: {NUM_WORKERS}")
    ...
    with ProcessPoolExecutor(
        ...
        # range(BUDGET) not range(BUDGET+1): the extra iteration exhausted the
        # optimizer budget and returned POP_SIZE identical cached candidates.
